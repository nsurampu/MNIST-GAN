...
# defining the Generator network function
...
def main_func():
    ...
    plt.show()
    # img1 = Image.fromarray(samples[0].reshape(28, 28), 'RGB')
    # img1.show()

    # result after 499th epoch
    plt.imshow(samples[999].reshape(28, 28))
    plt.show()
    # img2 = Image.fromarray(samples[499].reshape(28, 28), 'RGB')
    # img2.show()

pool = ThreadPool(processes=cores)
some_final = (pool.apply_async(main_func)).get()
